 ...
 * - 内层，每次执行k次
 */
public class L25Copy4 {
    public ListNode reverseKGroup(ListNode head, int k) {
        ListNode solder = new ListNode(0);
        solder.next = head;
        ListNode p = solder;

        ListNode preStart = null, start = null, pInner = null;
        int count = 0;
...
//        ListNode node = NodeUtils.arrayToList(1, 2, 3);
        printList(new L25Copy4().reverseKGroup(node, 3));
        printList(new L25Copy4().reverseKGroup(null, 3));
    }
}